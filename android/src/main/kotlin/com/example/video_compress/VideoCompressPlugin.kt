...
class VideoCompressPlugin : MethodCallHandler, FlutterPlugin {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "compressVideo" -> {
                ...
                val obj  = Utility(channelName).getMediaInfoJson(context,path);
                val widthOriginalVideo = obj.getInt("width");
                val heightOriginalVideo = obj.getInt("height");
                println("===================> $obj[\"width\"]");
                ...
                when (quality) {
                    ...
                    3 -> {
                        ...
                        videoTrackStrategy = DefaultVideoStrategy.Builder()
                                .keyFrameInterval(3f)
                                .bitRate(getCompressionResolution(widthOriginalVideo,heightOriginalVideo) * 4.toLong())
    ...
    fun getCompressionResolution(width:Int,height:Int) : Int {
        val res = if (width < height)  width else height;
        if (res<1440) return width*height;
        return 1440*2560;
    }
